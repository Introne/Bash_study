...
需要进一步加深理解！！！！  
首先需要理解两个概念：时域和频域  
时域：以时间为参考来描述动态变化的世界，比如，随着年纪的增长，我越来越好看  
频域：世界精止，更古不变！以音乐为例，普遍认为是琴弦随着时间的震动，音乐在声波上会呈现波动的曲线（时域），但在乐谱上不过一个个音符（频域），音符本身并未变化！

任何周期函数，都可以看作是不同振幅，不同相位正弦波的叠加。（王老师展示的关于f(n)的动图）

在第一个例子里我们可以理解为，利用对不同琴键不同力度，不同时间点的敲击，可以组合出任何一首乐曲。

而贯穿时域与频域的方法之一，就是传中说的傅里叶分析。傅里叶分析可分为傅里叶级数（Fourier Serie）和傅里叶变换

傅里叶级数的本质是将一个周期的信号分解成无限多分开的（离散的）正弦波

但是并非万事都是周期的，后面逐渐不理解了，还设计了欧拉公式等概念，后续如果再碰到这个概念就进一步琢磨
...
### pcre: Perl函数库 
PCRE(Perl Compatible Regular Expressions)是一个轻量级的Perl兼容的C库，包括perl 兼容的正则表达式库等。 
